...
def plot_earthquake_map_with_animation(earthquake_prone_regions, seismic_signals, shapefile_folder, output_file):
    ...
    circles = [Circle((0, 0), 0.1, color='blue', alpha=0.6) for _ in range(len(seismic_signals))]
    ...
    def update(frame):
        for i, (signal, circle) in enumerate(zip(seismic_signals, circles)):
            x = gdf.geometry.x.iloc[i]
            y = gdf.geometry.y.iloc[i]
            radius = 1e6 * (frame / len(signal))  # Adjust radius based on frame
            circle.center = (x, y)
